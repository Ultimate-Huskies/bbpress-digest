...
function bbp_digest_do_save_profile_fields( $user_id ) {
	...
	$bbp_digest_day = in_array( $_POST['bbp-digest-day'], range( 0, 6 ) ) ? $_POST['bbp-digest-day'] : '';
	$bbp_digest_forums = '';
	...
	foreach ( $meta as $meta_key => $new_meta_value ) {
		...
		if ( isset( $new_meta_value ) && strlen( $new_meta_value ) > 0 && '' == $meta_value )
			add_user_meta( $user_id, $meta_key, $new_meta_value, true );

		/* If the new meta value does not match the old value, update it. */
		elseif ( isset( $new_meta_value ) && strlen( $new_meta_value ) > 0 && $new_meta_value != $meta_value )
